...
To update the database use this command from the Web project folder (replace `Personas.DDD` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../Personas.DDD.Infrastructure/Personas.DDD.Infrastructure.csproj -s Personas.DDD.Web.csproj
```
